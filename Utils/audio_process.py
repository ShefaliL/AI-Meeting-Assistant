...
def convert_audio_to_wav(input_file:str) -> str:
    ...
    audio = audio.set_channels(1).set_frame_rate(16000)  # Set to mono and frame rate  and 16khz
...
def split_audio_into_chunks(wav_path:str, chunk_duration:int=10) -> list:
    audio = AudioSegment.from_wav(wav_path)#loading it here
    #now converting the chunk durations to minutes
    chunk_duration_ms = chunk_duration * 60 * 1000  # Convert seconds to milliseconds
